# Solutions to questions from module 2 homework

1) Within the execution for Yellow Taxi data for the year 2020 and month 12: what is the uncompressed file size (i.e. the output file yellow_tripdata_2020-12.csv of the extract task)?

> Execute the kestra flow given below with the following inputs, taxi: yellow, year: 2020 and month: 12. Once the ETL pipeline has finished its run, navigate to google cloud storage bucket and check the file size. **(134.5MiB)**
...
> Use scheduled backfiling to ingest all the CSV files for the year 2020, once the data has been ingested use the SQL query in BigQuery below to get the total number of rows for yellow taxi data. **24,648,499**

```sql
SELECT COUNT(*) AS yellow_2020_rows
FROM `starlit-effect-484216-a1.zoomcamp202625.yellow_tripdata`
...
> Similar to the previous question ingest data for green taxi for the year 2020, and execute the SQL query in BigQuery to get the total number of rows for green taxi data. **1,734,051**

```sql
SELECT COUNT(*) AS green_2020_rows
FROM `starlit-effect-484216-a1.zoomcamp202625.green_tripdata`
WHERE EXTRACT(YEAR FROM lpep_pickup_datetime) = 2020;
```

5) How many rows are there for the Yellow Taxi data for the March 2021 CSV file?

> Similar to the previous two questions, first ingest all CSV files for the year 2021 for yellow taxi data into the GCS bucket and the use the SQL query in BigQuery to find the total number of rows for yellow taxi data for march 2021. **1,925,152**

```sql
SELECT COUNT(*) FROM `starlit-effect-484216-a1.zoomcamp202625.yellow_tripdata` WHERE EXTRACT(YEAR FROM tpep_pickup_datetime) = 2021 AND EXTRACT(MONTH FROM tpep_pickup_datetime) = 03;
```

6) How would you configure the timezone to New York in a Schedule trigger?

> **Add a timezone property set to America/New_York in the Schedule trigger configuration** (from kestra documentation)